...
export class MatchStateManager {
  ...
  undoLastGoal(): boolean {
    const team = this.goalHistory.pop();
    ...
    let streakRed = 0;
    let streakBlue = 0;
    for (const t of this.goalHistory) {
      if (t === "red") {
        streakRed++;
        streakBlue = 0;
      } else {
    ...
    this.state.consecutiveGoalsRed = streakRed;
    this.state.consecutiveGoalsBlue = streakBlue;
  ...
  private tick(): void {
    if (!this.state.matchActive || this.state.isPaused) {
      this.lastTickAt = null;
      return;
    }
    const now = Date.now();
    const elapsedMs = this.lastTickAt ? Math.max(0, now - this.lastTickAt) : 1000;
    this.lastTickAt = now;

    this.state.timeRemainingMs = Math.max(0, this.state.timeRemainingMs - elapsedMs);
    this.recomputeIntense();
